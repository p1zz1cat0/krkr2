...
//
// 移植自 krkrz/krkr2 上游：
//   kirikiri2/trunk/kirikiri2/src/plugins/win32/shrinkCopy/main.cpp
// 作者 miahmie，许可证遵循吉里吉里本体（KrKr2）。
//
// 为 Layer 增加面积平均法（1/256 像素分辨率近似）的缩小专用复制方法：
//   - Layer.shrinkCopy(dleft, dtop, dwidth, dheight, src, sleft, stop, swidth, sheight)
//     1/2 以下缩小时比 stretchCopy/affineCopy 质量更高，但速度较慢，
//     适合工具用途或存档缩略图生成，不适合实时绘制。
//   - Layer.shrinkCopyFast(src, shrink_x, shrink_y)
...
// 上游为 Windows 调用约定声明；本工程无此宏时保持为空
...
struct ShrinkCopy : public LayerUtils
{
	...
	bool check()
	{
		if (sw <= 0|| sh <= 0 || dw <= 0 || dh <= 0 ||
			sw < (long)dw || sh < (long)dh) { diag = "invalid rect: this method only shrinks"; return false; }

		// 获取大小
		if (!GetLayerBufferAndSize(src, siw, sih, ps, spch)) { diag = "source layer has no readable image"; return false; }
		if (!GetLayerBufferAndSize(dst, diw, dih, pd, dpch)) { diag = "destination layer has no writable image"; return false; }
	...
	void copy() {
		...
		for (long x,   y=dsy; y < dey; y++,vi++,pl+=dpch) {
			...
			for (p=pl, x=dsx; x < dex; x++,hi++,p+=4) {
				...
				const int ox = w << 2;
				const int oy = h * spch;
				addPoint(sum, r- 4-spch,       hi->ta * vi->ta,  hi->tc * vi->tc); // 左上角
				addHorz (sum, r   -spch, w,    hu     * vi->ta,  hu     * vi->tc); // 上边
				addPoint(sum, r+ox-spch,       hi->ba * vi->ta,  hi->bc * vi->tc); // 右上角
				addVert (sum, r- 4,         h, hi->ta * vu,      hi->tc * vu    ); // 左边
				addRect (sum, r,         w, h, unit                             ); // 中央
				addVert (sum, r+ox,         h, hi->ba * vu,      hi->bc * vu    ); // 右边
				addPoint(sum, r- 4+oy,         hi->ta * vi->ba,  hi->tc * vi->bc); // 左下角
				addHorz (sum, r   +oy,   w,    hu     * vi->ba,  hu     * vi->bc); // 下边
				addPoint(sum, r+ox+oy,         hi->ba * vi->ba,  hi->bc * vi->bc); // 右下角
	...
	AvgT makeAvgTable(AvgInfoT *tbl, bool isHorz) {
		...
		work.unit = 256; // 1 像素的分辨率
		if (work.ratio <= 1.0/16) {
			// 1/16 以下时单点贡献可能超过分辨率，把 unit 也放大
...
struct LimitedShrink : public LayerUtils
{
	...
	static tjs_error (TJS_INTF_METHOD layerShrinkCopy)(tTJSVariant *result, tjs_int numparams, tTJSVariant **param, iTJSDispatch2 *dst)
	{
		...
		if (!inst.resize()) {
			LogPluginError("shrinkCopyFast: destination layer cannot be resized");
			return TJS_E_FAIL;
		}
		inst.copy();
		return TJS_S_OK;
	...
	bool check()
	{
		if (stepx <= 0 || stepy <= 0) { diag = "invalid shrink step (must be >= 1)"; return false; }
		if (!GetLayerBufferAndSize(src, siw, sih, ps, spch)) { diag = "source layer has no readable image"; return false; }
		if (!IsValidLayer(dst)) { diag = "destination is not a Layer with image"; return false; }
		return true;
	}
	bool resize() {
