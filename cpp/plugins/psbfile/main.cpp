...
static tjs_error getRoot(tTJSVariant *r, tjs_int n, tTJSVariant **p,
    ...
    if(self == nullptr || r == nullptr || self->getObjects() == nullptr)
...
static tjs_error load(tTJSVariant *r, tjs_int count, tTJSVariant **p,
    ...
    try {
        if((*p)->Type() == tvtString) {
            ttstr path{ **p };
            if(!self->loadPSBFile(path) || self->getObjects() == nullptr) {
                LOGGER->error("cannot load psb file : {}", path.AsStdString());
                loadSuccess = false;
            } else {
                PSB::registerRootResources(path, *self);
            }
        } else if((*p)->Type() == tvtOctet) {
            LOGGER->error("PSBFile::load stream is not implemented");
            loadSuccess = false;
        } else {
            return TJS_E_INVALIDPARAM;
        }
    } catch(const std::exception &error) {
        LOGGER->error("PSBFile::load failed: {}", error.what());
        loadSuccess = false;
    } catch(...) {
        LOGGER->error("PSBFile::load failed with an unknown exception");
        loadSuccess = false;
    }
    ...
    try {
        if(count == 0) {
            ...
            if(!psbFile->loadPSBFile(path) ||
               psbFile->getObjects() == nullptr) {
                delete psbFile;
                return TJS_E_FAIL;
            }
            PSB::registerRootResources(path, *psbFile);
        } else {
            return TJS_E_INVALIDPARAM;
        }
    } catch(const std::exception &error) {
        LOGGER->error("PSBFile construction failed: {}", error.what());
        delete psbFile;
        return TJS_E_FAIL;
    } catch(...) {
        LOGGER->error("PSBFile construction failed with an unknown exception");
        delete psbFile;
        return TJS_E_FAIL;
    }
    *result = psbFile;
