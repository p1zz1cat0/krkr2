...
copyRightBlueToLeftAlpha(tTJSVariant *result, tjs_int numparams, tTJSVariant **param, iTJSDispatch2 *lay)
{
	...
	layObj.FuncCall(0, TJS_W("update"), &updateHint, NULL, (tjs_int)0, (tjs_int)0, (tjs_int)dw, (tjs_int)dh);
	return TJS_S_OK;
}
...
fillAlpha(tTJSVariant *result, tjs_int numparams, tTJSVariant **param, iTJSDispatch2 *lay)
{
	...
	ncbPropAccessor layObj(lay);
	layObj.FuncCall(0, TJS_W("update"), &updateHint, NULL, (tjs_int)l, (tjs_int)t, (tjs_int)dw, (tjs_int)dh);
	return TJS_S_OK;
}
...
copyAlphaToProvince(tTJSVariant *result, tjs_int numparams, tTJSVariant **param, iTJSDispatch2 *lay)
{
	...
	ncbPropAccessor layObj(lay);
	layObj.FuncCall(0, TJS_W("update"), &updateHint, NULL, (tjs_int)l, (tjs_int)t, (tjs_int)w, (tjs_int)h);
	return TJS_S_OK;
}
...
clipAlphaRect(tTJSVariant *result, tjs_int numparams, tTJSVariant **param, iTJSDispatch2 *dst)
{
	...
	if (clr) {
		layObj.FuncCall(0, TJS_W("update"), &updateHint, NULL, (tjs_int)dl, (tjs_int)dt, (tjs_int)diw, (tjs_int)dih);
	} else {
		layObj.FuncCall(0, TJS_W("update"), &updateHint, NULL, (tjs_int)(dl+dx), (tjs_int)(dt+dy), (tjs_int)w, (tjs_int)h);
	}
	return TJS_S_OK;
none:
	// 領域範囲外で演算が行われない場合
