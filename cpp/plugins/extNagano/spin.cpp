...
{
	if(!Col1 || !Col2 || !Slope1 || !Slope2 || !Select)
		return TJS_E_FAIL;
	// FUN_10018070
		...
		for(tjs_int c = left; c < right; c++)
		{
			...
			if(sel == 0)
			{
				...
				if(srow >= 0 && srow < H)
				{
					...
					if(TJS_FAILED(ExtNaganoGetSrcScanLine(data->Src1, srow, (const void**)&src1)))
						return TJS_E_FAIL;
					dp[c] = src1[ Col1[c] ];
				}
				else dp[c] = 0;
			...
			else if(sel == 1)
			{
				...
				if(srow >= 0 && srow < H)
				{
					...
					if(TJS_FAILED(ExtNaganoGetSrcScanLine(data->Src2, srow, (const void**)&src2)))
						return TJS_E_FAIL;
					dp[c] = src2[ Col2[c] ];
				}
				else dp[c] = 0;
