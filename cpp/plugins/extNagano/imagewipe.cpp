...
//---------------------------------------------------------------------------
// Layer / パス指定のルール画像は extNagano::CreateRuleProvider で
// テクスチャ付き iTVPScanLineProvider へスナップショットする。
//---------------------------------------------------------------------------
...
void tTVPImageWipeTransHandler::Setup(pluginSafety::OperationBudget &budget)
{
	...
	for(tjs_int row = 0; row < Height; row++)
	{
		...
		if(!Rule || TJS_FAILED(ExtNaganoGetSrcScanLine(Rule, rrow, (const void**)&rule)))
			continue;
...
tjs_error TJS_INTF_METHOD tTVPImageWipeTransHandler::Process(tTVPDivisibleData *data)
{
	...
	for(tjs_int n = 0; n < data->Height; n++)
	{
		...
		if(TJS_FAILED(ExtNaganoGetDstScanLine(data->Dest, data->DestTop + n, (void**)&dest)))
			return TJS_E_FAIL;
		if(TJS_FAILED(ExtNaganoGetSrcScanLine(data->Src1, y, (const void**)&src1)))
			return TJS_E_FAIL;
		if(TJS_FAILED(ExtNaganoGetSrcScanLine(data->Src2, y, (const void**)&src2)))
			return TJS_E_FAIL;
		...
		if(Rule)
		{
			...
			if(RuleHeight > 0 && rrow >= RuleHeight) rrow = RuleHeight - 1;
			if(rrow < 0) rrow = 0;
			ExtNaganoGetSrcScanLine(Rule, rrow, (const void**)&rule);
		}
...
public:
			...
			iTVPBaseTransHandler ** handler)
	{
		...
		iTVPScanLineProvider *scpro =
			extNagano::CreateRuleProvider(options, imagepro, src1h);
