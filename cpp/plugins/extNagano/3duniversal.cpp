...
public:
	...
	tjs_error AddRef()  { RefCount++; return TJS_S_OK; }
	tjs_error Release() { if(RefCount == 1) delete this; else RefCount--; return TJS_S_OK; }
};
//---------------------------------------------------------------------------
		...
		tRuleScanLineProvider **out)
{
	*out = 0;
	...
	else
	{
		...
		loaded->GetWidth(&rw);
		loaded->GetHeight(&rh);
		if(!extNagano::CheckImageSize(rw, rh)) {
			loaded->Release();
			return TJS_E_FAIL;
		}
...
public:
		  ...
		  Bound1(ClampI(bound1, 0, 0x7f)), Bound2(ClampI(bound2, 0, 0x7f))
	{
		...
		{
			OutBuf  = (tjs_uint32*)malloc(sizeof(tjs_uint32) *
			                              (size_t)Width * (size_t)Height);
			RuleBuf = (tjs_uint32*)malloc(sizeof(tjs_uint32) *
			                              (size_t)Width * (size_t)Height);
		}
...
void tTVP3duniversalTransHandler::CopySrcToOut(iTVPScanLineProvider *src)
{
	...
	{
		const void *p = 0;
		if(TJS_FAILED(ExtNaganoGetSrcScanLine(src, y, &p)) || !p) continue;
		...
		const tjs_int *moveTable, const tjs_int *opaTable)
{
	...
	for(tjs_int y = 0; y < Height; y++)
	{
		...
		if(TJS_FAILED(ExtNaganoGetSrcScanLine(src, y, (const void**)&sp)) || !sp) continue;
