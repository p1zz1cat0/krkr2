		...
		iTVPScanLineProvider *src, tjs_uint32 *dst, tjs_int rx, tjs_int ry)
{
	...
	{
		const tjs_uint32 *in;
		if(TJS_FAILED(ExtNaganoGetSrcScanLine(src, y, (const void**)&in))) return;
...
tjs_error TJS_INTF_METHOD tTVPBlurFadeTransHandler::Process(tTVPDivisibleData *data)
{
	...
	for(tjs_int n = 0; n < data->Height; n++)
	{
		...
		if(alpha == 0)
		{
			...
			if(TJS_FAILED(ExtNaganoGetSrcScanLine(data->Src1, y, (const void**)&src1))) return TJS_E_FAIL;
		...
		if(alpha == 255)
		{
			...
			if(TJS_FAILED(ExtNaganoGetSrcScanLine(data->Src2, y, (const void**)&src2))) return TJS_E_FAIL;
		...
		else { if(TJS_FAILED(ExtNaganoGetSrcScanLine(data->Src1, y, (const void**)&s1))) return TJS_E_FAIL; }
		if(useBlur2) s2 = Buffer2 + y * Width;
		else { if(TJS_FAILED(ExtNaganoGetSrcScanLine(data->Src2, y, (const void**)&s2))) return TJS_E_FAIL; }
