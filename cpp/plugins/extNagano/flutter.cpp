...
{
	if(!BackBuf || !data || !data->Src1)
		return;
	for(tjs_int y = 0; y < Height; y++)
	{
		const tjs_uint32 *s1;
		if(TJS_FAILED(ExtNaganoGetSrcScanLine(data->Src1, y, (const void**)&s1)))
			return;
...
tjs_error TJS_INTF_METHOD tTVPFlutterTransHandler::Process(tTVPDivisibleData *data)
{
	...
	if(phase == 0)
	{
		...
		for(tjs_int n = 0; n < data->Height; n++)
		{
			...
			if(TJS_FAILED(ExtNaganoGetDstScanLine(data->Dest, data->DestTop + n, (void**)&dest)))
				return TJS_E_FAIL;
			if(TJS_FAILED(ExtNaganoGetSrcScanLine(data->Src1, data->Top + n, (const void**)&src1)))
				return TJS_E_FAIL;
	...
	if(phase == W + H)
	{
		...
		for(tjs_int n = 0; n < data->Height; n++)
		{
			...
			if(TJS_FAILED(ExtNaganoGetDstScanLine(data->Dest, data->DestTop + n, (void**)&dest)))
				return TJS_E_FAIL;
			if(TJS_FAILED(ExtNaganoGetSrcScanLine(data->Src2, data->Top + n, (const void**)&src2)))
				return TJS_E_FAIL;
			tjs_uint32 *dp = dest + data->DestLeft;
	...
	for(tjs_int n = 0; n < RH; n++)
	{
		...
		tjs_uint32 *destline;
		if(TJS_FAILED(ExtNaganoGetDstScanLine(data->Dest, data->DestTop + n, (void**)&destline)))
			return TJS_E_FAIL;
		...
		if(y < CS)
		{
			...
			const tjs_uint32 *s2;
			if(TJS_FAILED(ExtNaganoGetSrcScanLine(data->Src2, y, (const void**)&s2))) return TJS_E_FAIL;
			s2 += Left;
		...
		else
		{
			...
			if(j < rbound)
			{
				...
				const tjs_uint32 *s1;
				if(TJS_FAILED(ExtNaganoGetSrcScanLine(data->Src1, y - CS, (const void**)&s1))) return TJS_E_FAIL;
				s1 += Left;
