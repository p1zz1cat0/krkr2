...
public:
    ...
    void drawToLayer(::tTJSNI_BaseLayer* layer, emotelimit lim, ::tTJSNI_BaseLayer* maskLayer);
    void drawToBitmap(iTVPBaseBitmap* target, const tTVPRect& targetClip, emotelimit lim,
                      ::tTJSNI_BaseLayer* maskLayer = nullptr);
...
private:
    ...
    // opengl
    void resizeOpenGL();
    bool uploadIconGLTexture();
    GLuint selftexture = 0; // 自身图片纹理
    bool _iconGlTextureUploaded = false;
    mutable tTVPBaseTexture* _cachedSourceBitmap = nullptr;
    mutable emoteicon* _cachedSourceIcon = nullptr;
    mutable tjs_int _cachedSourceWidth = 0;
    mutable tjs_int _cachedSourceHeight = 0;
    iTVPBaseBitmap* getSourceBitmap() const;
    void drawPatchCommon(const emotelimit& lim, iTVPBaseBitmap* sourceBitmap,
