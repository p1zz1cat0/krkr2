...
namespace {
    ...
    softwareMethodForPrivateMotionGLLLike_0x6DD56C(int blendLowNibble) {
        // See softwareMethodForD3DBlend: target buffer is alpha-aware (A=0
        // clear). Use dest-alpha blends so capture is not RGB-with-A=0.
    ...
    bool operatePrivateMotionGLLMeshLike_0x6DD56C(
        ...
        for(int y = 0; y < item.meshDivY - 1; ++y) {
            ...
            for(int x = 0; x < item.meshDivX - 1; ++x) {
                ...
                const double texU1 = sourceRect.left +
                    std::max(std::floor(srcW * u0) + 1.0,
                             std::floor(srcW * u1));
                const double texV0 =
                    sourceRect.top + std::floor(srcH * v0);
                const double texV1 = sourceRect.top +
                    std::max(std::floor(srcH * v0) + 1.0,
                             std::floor(srcH * v1));
