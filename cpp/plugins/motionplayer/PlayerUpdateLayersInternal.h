...
namespace {
    inline void
    copyPackedColorsToBytes(uint8_t (&colorBytes)[16],
                            const std::array<std::uint32_t, 4> &packedColors) {
        std::memcpy(colorBytes, packedColors.data(),
    ...
    inline void populateTimelinePayloadFromFrameState(
        StateT &localState, const motion::internal::FrameContentState &state) {
        ...
        localState.opacity =
            static_cast<int>(std::clamp(state.opacity * 255.0, 0.0, 255.0));
    ...
    inline void
    copyDeltaBlockToAccum(motion::detail::MotionNode::AccumulatedState &accum,
    ...
    // PlayerInternal.h (motion::internal namespace) which MotionNode.h cannot
    // include.
    inline void
    populateSlotFromState(motion::detail::MotionNode::ClipSlot &slot,
        ...
        slot.oy = s.oy;
        slot.width = s.width;
        slot.height = s.height;
        slot.opacity = s.opacity;
        slot.angle = s.angle;
        slot.scaleX = s.scaleX;
        slot.scaleY = s.scaleY;
        slot.slantX = s.slantX;
        slot.slantY = s.slantY;
        slot.flipX = s.flipX;
        slot.flipY = s.flipY;
        slot.blendMode = s.blendMode;
        slot.packedColors = s.packedColors;
        slot.meshBezierPoints = s.meshBezierPoints;
        slot.ccc.x = s.ccc.x;
        slot.ccc.y = s.ccc.y;
        slot.acc.x = s.acc.x;
        slot.acc.y = s.acc.y;
        ...
        slot.motionDt = s.motionDt;
        slot.motionFlags = s.motionFlags;
        slot.motionDofst = s.motionDofst;
        slot.motionDocmpl = s.motionDocmpl;
        slot.motionTimeOffset = s.motionTimeOffset;
        slot.motionDtgt = s.motionDtgt;
        slot.prtTrigger = s.prtTrigger;
        slot.prtFmin = s.prtFmin;
        slot.prtF = s.prtF;
        slot.prtVmin = s.prtVmin;
        slot.prtV = s.prtV;
        slot.prtAmin = s.prtAmin;
        slot.prtA = s.prtA;
        slot.prtZmin = s.prtZmin;
        slot.prtZ = s.prtZ;
        ...
        slot.action = s.action;
        slot.hasSync = s.hasSync;
    ...
    inline void applyLocalTransform(Affine2x3 &a, bool flipX, bool flipY,
    ...
    inline void applyLocalTransform(Affine2x3 &a,
        ...
        applyLocalTransform(a, state.flipX, state.flipY, state.angle,
                            state.scaleX, state.scaleY, state.slantX,
                            state.slantY, state.transformOrder);
    }
    ...
    inline void applyLocalTransform(Affine2x3 &a,
        ...
        applyLocalTransform(a, node.accumulated.flipX, node.accumulated.flipY,
                            node.accumulated.angle, node.accumulated.scaleX,
                            node.accumulated.scaleY, node.accumulated.slantX,
                            node.accumulated.slantY, node.transformOrder);
    }
    ...
    // parent.meshType != 0.
    //
    // Child positions stay parent-local. Only content.mesh.bp (unit 4×4)
    // drives the surface sample — dense meshControlPoints is draw geometry.
    inline void sub_69AE74_meshDeform(const motion::detail::MotionNode &parent,
                                      motion::detail::MotionNode &node) {
        if(parent.meshType != 1 || (parent.meshFlags & 1) == 0 ||
           !parent.accumulated.active || !parent.hasSource)
            return;
        if(parent.interpolatedCache.meshBezierPoints.size() != 32)
            return;
        ...
        const double childSecondary = parent.coordinateMode != 0
        ...
        auto evalBezierPatch = [](const float *mesh, float u, float v,
            ...
            const float bu[4] = { su * su * su, 3.0f * su * su * u,
                                  3.0f * su * u * u, u * u * u };
            const float bv[4] = { sv * sv * sv, 3.0f * sv * sv * v,
                                  3.0f * sv * v * v, v * v * v };
            outX = 0;
            outY = 0;
            for(int i = 0; i < 16; ++i) {
        ...
        for(size_t i = 0; i < 32; ++i) {
            unitPatch[i] = static_cast<float>(
                parent.interpolatedCache.meshBezierPoints[i]);
        }
        const float *sampleMesh = unitPatch.data();

        float defX = static_cast<float>(normX);
        float defY = static_cast<float>(normY);
        evalBezierPatch(sampleMesh, defX, defY, defX, defY);
        ...
        if((parent.meshFlags & 2) != 0 && (node.inheritFlags & 0x10) != 0) {
            const float eps = 0.0001f;
            float x1, y1, x2, y2, x3, y3, x4, y4;
            // Sample around the pre-deform unit coords for a local jacobian.
            const float su = static_cast<float>(normX);
            const float sv = static_cast<float>(normY);
            evalBezierPatch(sampleMesh, su - eps, sv, x1, y1);
            evalBezierPatch(sampleMesh, su + eps, sv, x2, y2);
            evalBezierPatch(sampleMesh, su, sv - eps, x3, y3);
            evalBezierPatch(sampleMesh, su, sv + eps, x4, y4);
        ...
        if((parent.meshFlags & 4) != 0 && (node.inheritFlags & 0x60) != 0) {
            const float eps = 0.0001f;
            float x1, y1, x2, y2, x3, y3, x4, y4;
            const float su = static_cast<float>(normX);
            const float sv = static_cast<float>(normY);
    ...
    inline void
    sub_6BAA10_groundCorrection(motion::detail::MotionNode &node,
