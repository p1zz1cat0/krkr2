...
namespace motion {
    namespace internal {
        ...
        inline bool decodePsbPixelResource(
            ...
            // Prefer explicit RL; also auto-detect when payload is smaller than
            // raw RGBA (common for Yuzusoft/E-mote .mtn textures when the
            // "compress" field is missing or not propagated).
            const bool looksCompressed =
                pixelResource.data.size() < expectedRgbaBytes;
            if(isRLCompressed || looksCompressed) {
            ...
            if(pixelResource.data.size() >= expectedRgbaBytes) {
        ...
        resolveMotion(detail::PlayerRuntime &runtime, const ttstr &name,
            ...
            if(detail::resolveExistingPath(candidates, resolved)) {
                ...
                // Forensic: a motion cross-reference that resolved to no
                // storage. Shows what the resolver was asked for and how much
                // was already cached when the storage load was attempted.
                LOGGER->warn(
                    "emote.motion.miss key='{}' cached={} storageResolved=0",
                    requestKey, runtime.motionsByKey.size());
            }
        ...
        struct FrameContentState {
            ...
            std::array<std::uint32_t, 4> packedColors{ 0xFF808080u, 0xFF808080u,
                                                       0xFF808080u,
                                                       0xFF808080u };
            // mask 0x02000000: normalized 4x4 Bezier control points
            // stored as 16 interleaved x/y pairs under content.mesh.bp.
            std::vector<double> meshBezierPoints;
        ...
        mergeFrameContent(const std::shared_ptr<PSB::PSBDictionary> &content,
            ...
            if(mask & 0x02000000) {
                if(auto mesh = psbDictionaryValue(content, "mesh")) {
                    if(auto points = psbDictionaryList(mesh, "bp")) {
                        state.meshBezierPoints.reserve(points->size());
                        for(size_t index = 0; index < points->size(); ++index) {
                            if(auto value = psbNumberValue((*points)[index])) {
                                state.meshBezierPoints.push_back(*value);
                            }
                        }
                    }
                }
        ...
        parseFrame(const std::shared_ptr<PSB::PSBDictionary> &frame,
            ...
            // 参考 sdl3 emoteframe::hasContent（有 content 键即可绘制，与 type
            // 解耦）
        ...
        FrameContentState advanceNodeFrameSelectionLike_0x6926B4(
        ...
        inline const PSB::PSBResource *findPSBResourceBySourceName(
            ...
            // blank/w:h:ox:oy — pure layout rect used by e-mote mesh parents
            // (e.g. eye_pos blank/155:136:77:68). No pixel resource; only
            // sets clip geometry so sub_69AE74 / mesh inv stay in real units.
            // Without this, clipW=0 → unit coords explode → 五官 degenerates
            // to dots/lines under mesh parents.
            if(source.rfind("blank/", 0) == 0) {
                const auto body = source.substr(6);
                double vals[4] = { 0.0, 0.0, 0.0, 0.0 };
                size_t start = 0;
                int count = 0;
                while(start <= body.size() && count < 4) {
                    const size_t colon = body.find(':', start);
                    const auto token = colon == std::string::npos
                        ? body.substr(start)
                        : body.substr(start, colon - start);
                    if(!token.empty()) {
                        try {
                            vals[count] = std::stod(token);
                        } catch(...) {
                            vals[count] = 0.0;
                        }
                    }
                ...
                if(count >= 2 && vals[0] > 0.0 && vals[1] > 0.0) {
                    outWidth = static_cast<int>(vals[0]);
                    outHeight = static_cast<int>(vals[1]);
                    if(count >= 3) {
                        outOriginX = vals[2];
                    }
                    if(count >= 4) {
