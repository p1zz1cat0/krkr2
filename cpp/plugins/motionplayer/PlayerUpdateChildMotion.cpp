...
namespace motion {
    void Player::updateLayersPhase3_MotionSubNode(double currentTime) {
        ...
        for(size_t i = 1; i < nodes.size(); ++i) {
            ...
            // Player* Child Player is pre-created in buildNodeTree (sub_6B3C78
            // case 3).
            {
                ...
                // Binary: calls cleanup (sub_6C0DE8, sub_6B56F8), releases TJS
                // variants, then goes to LABEL_3 (next loop iteration),
                // SKIPPING frameProgress/updateLayers.
                if(mn.activeSlot().done) {
                    // Binary cleanup at 0x6BE328..0x6BE354:
                    // 1. child._allplaying = false (player+1099)
                    // 2. sub_6C0DE8(child+1296) — resets timeline keyframe
                    // cache
                    // 3. sub_6B56F8(child) — releases layer IDs for all
                    // non-root nodes,
                    ...
                    if(child._runtime) {
                        ...
                        // sub_6B56F8: release layer IDs for non-root nodes,
                        // keep the constructor-created root, and clear the
                        // label map.
                        child.resetNodeTreeForBuildLike_0x6B56F8();
                    }
                    continue; // skip to next iteration — binary goes to
                              // LABEL_3, not LABEL_18
                }

                {
                    ...
                    if(!src.empty()) {
                        ...
                        if(emoteLikeReinit) {
                            ...
                            // (0x6BE21C reads it once and uses it unchanged
                            // throughout). Slot flip is managed elsewhere in
                            // the clip evaluation pipeline.

                            // Resolve motion and play (0x6BE3B4..0x6BE46C)
                            // Aligned to libkrkr2.so sub_6BE0C0 + sub_697D34:
                            // split src by "/" into segments.
                            // - 1 segment: setChara(segment[0]), then
                            // Player_play(raw src)
                            // - otherwise: setChara(segment[1]), then
                            // Player_play(segment[2])
                            //
                            // This is important for paths like
                            // "motion/m2cheeseware_logo/icon25": the native
                            // code ignores the first "motion" prefix and uses
                            // chara="m2cheeseware_logo", motion="icon25".
                            {
                                ...
                                while(start <= src.size()) {
                                    ...
                                    segments.push_back(
                                        src.substr(start, slashPos - start));
                                    start = slashPos + 1;
                                }
                                ...
                                if(segments.size() <= 1) {
                                    ...
                                    child.onFindMotion(
                                        detail::widen(segments[1]),
                                        mn.activeSlot().motionFlags | v12);
                                }
                            }
                            // Stealth motion (0x6BE41C..0x6BE44C): binary reads
                            // from CHILD player+776, plays with flag 16, then
                            // clears child+776.
                            if(!child._stealthMotion.IsEmpty()) {
                                child.onFindMotion(child._stealthMotion,
                                                   PlayFlagStealth);
                            ...
                            if(detail::logoSnapshotMarkEnabledForPath(
                                   motionPath) &&
                               motionPath.find("m2logo.mtn") !=
                                   std::string::npos &&
                               currentTime >= 0.0 && currentTime <= 50.0) {
                                std::fprintf(
                                    stderr,
                                    "SNAPPLAY frame=%.3f nodeIndex=%d src=%s "
                                    "childMotionKey=%s childActiveMotion=%s "
                                    "childNodesBuilt=%d childPlaying=%d\n",
                                    currentTime, mn.index, src.c_str(),
                            ...
                            // Time sync from parent loop time
                            // (0x6BE478..0x6BE4E8) Binary checks both
                            // _allplaying && _queuing (0x6BE478)
                            if(child._allplaying && child._queuing) {
                                // Binary at 0x6BE49C: childTime = player+1120 -
                                // slot+8 + slot+376 = _frameLoopTime -
                                // clipStartTime + motionTimeOffset
                                ...
                                if(_frameLastTime < 0.0) {
                                    ...
                                    // Binary reads child+1136 (_loopTime) and
                                    // child+1128 (_cachedTotalFrames)
                                    double loopEnd = child._loopTime;
                                    if(loopEnd >= 0.0) {
                                        childTime = detail::
                                            wrapTimelineCurrentTimeValue(
                                                childTime,
                                ...
                                // Binary: writes unclamped time to player+1120
                                // (0x6BE4D4)
                                child._frameLoopTime = childTime;
                                if(childTime > totalFrames)
                                    childTime = totalFrames;
                                // Binary: writes clamped time to player+456
                                // (0x6BE4E4)
                                child._clampedEvalTime = childTime;
                                // Binary at 0x6BE4E8: writes word 0x0101 to
                                // child+480, setting both _queuing (byte+480)
                                // and _allplaying (byte+481) simultaneously.
                                // Does NOT iterate timelines.
                                child._allplaying = true;
                                child._queuing = true;
                                // Binary: if (!*(byte*)(v4 + 480)) — checks
                                // _queuing (0x6BE4EC)
                                if(!_queuing) {
                    ...
                    // Binary at 0x6BE534 unconditionally proceeds to
                    // angle/state propagation (no activeMotion guard). Only
                    // guard for null runtime.
                    if(!child._runtime)
                        goto label_18;
                    ...
                    // Dual-slot crossfade angle interpolation
                    // (0x6BE85C..0x6BEC9C) When crossfading between two clip
                    // slots, blend dofst (v37) between old and new slot values
                    // using time-based ratio.
                    double v37 = dofst;
                    ...
                    if(angleMode != 0) {
                        // Case 2→3 fallthrough: binary at 0x6BE664 checks child
                        // player+608
                        // (_noUpdateYet). If set, case 2 falls through to
                        // LABEL_83 (case 3 logic) because on the first frame
                        // there's no delta position yet.
                        int effectiveMode = angleMode;
                        if(angleMode == 2 && child._noUpdateYet) {
                            effectiveMode =
                                3; // fallthrough to case 3 (0x6BE664→0x6BE668)
                        }
                        ...
                        switch(effectiveMode) {
                            ...
                            case 2: { // atan2 from delta position (0x6BE8C4)
                                // Binary uses v37 (potentially interpolated)
                                // not raw dofst
                                ...
                                if(mn.coordinateMode == 1) {
                                    ...
                                    // Binary: non-0/non-1 coordinateMode →
                                    // LABEL_129
                                    hasAngle = true;
                                    break;
                                }
                                computedAngle = v37 +
                                    std::atan2(dy_comp, dx_comp) * 360.0 /
                                        6.28318531;
                                hasAngle = true;
                                break;
                            }
                            case 3: { // Interpolated atan2 (LABEL_83:
                                      // 0x6BE668..0x6BE79C)
                                // Binary: guard: crossfading && !otherSlotDone
                                // (0x6BE680). If guard fails → hasAngle=false
                                // (LABEL_119). Otherwise: compute ratio from
                                // parent time, call sub_69A4D4 twice (at t and
                                // t+0.0001) for finite-difference derivative,
                                ...
                                double denom = otherStart - currentStart;
                                // Binary divides directly without zero guard
                                // (0x6BE6D0)
                                double ratio =
                                    (parentTime - currentStart) / denom;
                                double t2 = ratio + 0.0001;
                                if(t2 >= 1.0)
                                    ratio = 0.9999;
                                t2 = std::min(t2, 1.0);
                                // sub_69A4D4: interpolate between slot
                                // positions. src = currentSlot+96 = current
                                // evaluated position dst = otherSlot+96 =
                                // position from before crossfade
                                ...
                                // Use crossfade slot positions: src=current,
                                // dst=other (saved at flip) Binary reads full
                                // {x,y,z} from active slot (a3+96..112).
                                double src[3] = { slot.x, slot.y,
                                                  mn.activeSlot().z };
                                double dst[3] = { mn.otherSlot().x,
                                                  mn.otherSlot().y,
                                                  mn.otherSlot().z };
                                double out1[3] = {}, out2[3] = {};
                                interpolatePosition69A4D4(
                                    cccCurve, dst, src, out1, mn.coordinateMode,
                                    cpCurve, ratio);
                                interpolatePosition69A4D4(
                                    cccCurve, dst, src, out2, mn.coordinateMode,
                                    cpCurve, t2);
                                // Pick dx/dy based on coordinateMode
                                // (0x6BE72C..0x6BE740)
                                double dx_comp, dy_comp;
                                if(mn.coordinateMode == 1) {
                                    dx_comp = out2[0] - out1[0];
                                    dy_comp = out2[2] - out1[2];
                                } else if(mn.coordinateMode == 0) {
                                    dx_comp = out2[0] - out1[0];
                                    dy_comp = out2[1] - out1[1];
                                } else {
                                    hasAngle = true;
                                    break; // LABEL_129
                                }
                            ...
                            case 4: { // Target node lookup (0x6BE7B4)
                                // Binary: hasAngle is only set to true when
                                // target found and angle computed. LABEL_119
                                // sets hasAngle=false.
                                const auto &dtgt = mn.activeSlot().motionDtgt;
                                if(dtgt.empty())
                                    break; // LABEL_119: hasAngle=false
                                int targetIdx = findNodeByLabel(
                                    _runtime->nodeLabelMap, dtgt);
                                if(targetIdx < 0)
                                    break; // LABEL_119: hasAngle=false
                                const auto &target = nodes[targetIdx];
                                double dy_comp, dx_comp;
                                if(mn.coordinateMode == 1) {
                                    dy_comp = target.accumulated.posZ -
                                        mn.accumulated.posZ;
                                    dx_comp = target.accumulated.posX -
                                        mn.accumulated.posX;
                                } else if(mn.coordinateMode == 0) {
                                    dy_comp = target.accumulated.posY -
                                        mn.accumulated.posY;
                                    dx_comp = target.accumulated.posX -
                                        mn.accumulated.posX;
                                } else {
                        ...
                        // Binary normalizes per-case (cases 2,3,4 each have
                        // inline loops). Case 1 does NOT normalize. Skip
                        // normalization for case 1.
                        if(effectiveMode != 1) {
                            while(computedAngle < 0.0)
                                computedAngle += 360.0;
                            while(computedAngle >= 360.0)
                                computedAngle -= 360.0;
                        }
                    }
                    ...
                    if(originX != 0.0 || originY != 0.0) {
                        ...
                        const double vx = mn.accumulated.m12 * negOY -
                            originX * mn.accumulated.m11;
                        // v80 = m22*negOY - originX*m21 (0x6BE9E4)
                        const double vy = mn.accumulated.m22 * negOY -
                            originX * mn.accumulated.m21;
                        if(mn.coordinateMode == 1) {
                            posX += vx;
                            posZ += vy;
                        } else {
                    ...
                    // === State propagation to child root node
                    // (0x6BEA18..0x6BEB74) ===
                    if(child._runtime && !child._runtime->nodes.empty()) {
                        ...
                        // Active/visible overrides are consumed by the next
                        // child Player_updateLayers root memcpy at 0x6BB4D4.
                        ...
                        // Binary: *(_DWORD *)(v16 + 1156) = *(_DWORD *)(v10 +
                        // 100) Reads node+100 (colorBytes[0..3] packed as
                        // uint32 RGBA), writes to child player+1156
                        // (_parentColorPacked). NOT a blend mode field.
                        {
                            uint32_t packed;
                            std::memcpy(&packed, &mn.colorBytes[0],
                                        sizeof(uint32_t));
                        ...
                        // Binary at 0x6BEA98: if isEmoteMode, call
                        // Player_initEmoteMotion(child, 2) This syncs emote
                        // bone state. Emote mode is not used in web port.

                        // === Angle → child (0x6BEAA8..0x6BEB08) ===
                        if(hasAngle) {
                            if(child._runtime->isEmoteMode) {
                                // Emote mode: normalize angle [0,360), set
                                // player+464, reinit
                        ...
                        // Binary at 0x6BEB90: condition is hasAngle ||
                        // angle==accAngle || child._directEdit (player+482).
                        // When _directEdit is true, direct-copy path is taken.
                        if(hasAngle || computedAngle == mn.accumulated.angle ||
                           child._directEdit) {
                            ...
                            // Rotate by (computedAngle - accumulated.angle)
                            // (0x6BEBC8..0x6BEC4C)
                            double delta =
                        ...
                        cr.delta.dirty = true;
                        // Note: clip chain propagation is done in label_18
                        // below, which ALL paths (active + inactive) fall
                        // through to.
                    }
                }
            ...
            if(auto *childP = mn.getChildPlayer()) {
                ...
                if(detail::logoSnapshotMarkEnabledForPath(motionPath) &&
                   ...
                   currentTime >= 30.0 && currentTime <= 50.0) {
                    ...
                    std::fprintf(
                        stderr,
                        "SNAPCHILD phase=runtime frame=%.3f nodeIndex=%d "
                        "src=%s parameterizeIndex=%d childActiveMotion=%s "
                        "childMotionKey=%s childClip=%s childAllPlaying=%d "
                        "childQueuing=%d childNodesBuilt=%d childNodeCount=%zu "
                        "childNeedsAssignImages=%d\n",
                        currentTime, mn.index,
                        mn.activeSlot().src.empty()
                            ? "<none>"
                        ...
                        child._allplaying ? 1 : 0, child._queuing ? 1 : 0,
                        child._runtime && child._runtime->nodes.size() > 1 ? 1
                                                                           : 0,
                ...
                if(child._runtime && !child._runtime->nodes.empty()) {
                    ...
                    // e-mote 子 Player 自有 nodes[]；父树 node
                    // 下标不能写入子树。
                    if(emoteLike) {
                    ...
                    // Binary 0x6BE29C: propagates node+1952 (forceVisible) to
                    // child root
                ...
                const auto inheritedWrapperLim =
                    (i < _runtime->perNodeEvalData.size())
                        ? _runtime->perNodeEvalData[i].evalLim
                        : _runtime->logicalScreen;
                child._runtime->wrapperLim =
                    detail::effectiveNodeLimLike_REF(
                        inheritedWrapperLim, mn.clipW, mn.clipH, mn.originX,
                        mn.originY);
                // libkrkr2.so label_18：子 Player 每帧
                // frameProgress+updateLayers。
                if(emoteLike) {
                    child._externalMeshParents.clear();
                    int meshParentIndex = mn.meshParentIndex;
                    for(int guard = 0;
                        meshParentIndex >= 0 &&
                            meshParentIndex < static_cast<int>(nodes.size()) &&
                            guard < 256;
                        ++guard) {
                        const auto &meshParent = nodes[static_cast<size_t>(
                            meshParentIndex)];
                        child._externalMeshParents.push_back(&meshParent);
                        if(meshParent.meshParentIndex == meshParentIndex) {
                            break;
                        }
