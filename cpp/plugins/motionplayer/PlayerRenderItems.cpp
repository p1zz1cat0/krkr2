...
namespace {
    ...
    // Detect parent/child Player cycles while merging nested motion render
    // lists.
    thread_local std::unordered_set<motion::Player *>
        gActivePrepareRenderPlayers;
...
namespace motion {
    ...
    void Player::appendPreparedRenderItems() {
        ...
        // 与 updateLayersPhase3_Visibility 一致：emote 须含 nodeType
        // 11（icon/口型）。
        ...
        auto climbVisibleAncestorChain = [&](int startNodeIndex, auto &&visit) {
        ...
        for(size_t i = 0; i < nodes.size(); ++i) {
            ...
            const bool mergedBoundedChild =
                !_preview && shouldMergeEmoteBoundedChild(node);
            // Motion sub-players are progressed even when their wrapper node
            // is inactive (PlayerUpdateChildMotion's shared label_18 path).
            // E-mote head/face wrappers commonly use that shape, so merge the
            // bounded child before applying the wrapper's active gate; the
            ...
            if(!node.accumulated.active)
                continue;
            if(!_preview) {
                // e-mote：只 merge 有界 body/head/face 子树（见
                // shouldMergeEmoteBoundedChild）。其余 type=3/4 仍走
                // 非 emote 全量路径，避免粒子等 prepare 风暴。
                if(!emoteLikePrepare) {
            ...
            // 物理未运行时只跳过「前髪双影」副本：src/#custom/1 与
            // 前髪00 同尺寸全图重叠。#custom/4/5 是长后发主体（後ろ髪00
            // 仅短底座），#custom/7/8 是眉追加，切勿整树隐藏 #custom/。
            {
                const auto &src = node.interpolatedCache.src;
                const bool isFrontHairSwayCopy =
                    src == "src/#custom/1" ||
                    (src.rfind("src/#custom/", 0) == 0 &&
                     node.layerName.find("髪揺れ_前髪") != std::string::npos);
                if(isFrontHairSwayCopy) {
                    continue;
                }
            }
            ...
            const int skipTypeBitmask =
                emoteLikePrepare ? 6153 : (_preview ? 1097 : 1089);
            entry.skipFlag0 = ((skipTypeBitmask & (1 << node.nodeType)) == 0);
