        ...
        if(detail::logoChainTraceEnabled(_runtime->activeMotion)) {
            ...
            for(const auto &node : nodes) {
                ...
                detail::logoChainTraceLogf(
                    ...
                    currentTime,
                    "nodeIndex={} label={} type={} parent={} src={} "
                    "inherit=0x{:x} indep={} "
                    "interp[x={:.3f},y={:.3f},ox={:.3f},oy={:.3f},w={:.3f},h={:"
                    ".3f},opacity={:.6f},angle={:.3f},scale=({:.6f},{:.6f}),"
                    "slant=({:.6f},{:.6f}),flip=({},{}) blend={}] "
                    "local[pos=({:.3f},{:.3f},{:.3f}),angle={:.3f},scale=({:."
                    "6f},{:.6f}),slant=({:.6f},{:.6f}),flip=({},{}) "
                    "opacity={},blend={}] "
                    "parentAccum[pos=({:.3f},{:.3f},{:.3f}),scale=({:.6f},{:."
                    "6f}),slant=({:.6f},{:.6f}),matrix=({:.6f},{:.6f},{:.6f},{:"
                    ".6f}),opacity={},blend={}] "
                    "accum[pos=({:.3f},{:.3f},{:.3f}),scale=({:.6f},{:.6f}),"
                    "slant=({:.6f},{:.6f}),matrix=({:.6f},{:.6f},{:.6f},{:.6f})"
                    ",opacity={},blend={},active={},visible={}]",
                    node.index,
                    node.layerName.empty() ? std::string("<root>")
                                           : node.layerName,
                    node.nodeType, node.parentIndex,
                    ic.src.empty() ? std::string("<none>") : ic.src,
                    node.inheritFlags, _independentLayerInherit ? 1 : 0, ic.x,
                    ic.y, ic.ox, ic.oy, ic.width, ic.height, ic.opacity,
                    ic.angle, ic.scaleX, ic.scaleY, ic.slantX, ic.slantY,
                    ic.flipX ? 1 : 0, ic.flipY ? 1 : 0, ic.blendMode, ls.posX,
                    ls.posY, ls.posZ, ls.angle, ls.scaleX, ls.scaleY, ls.slantX,
                    ls.slantY, ls.flipX ? 1 : 0, ls.flipY ? 1 : 0, ls.opacity,
                    ls.blendMode, pc.posX, pc.posY, pc.posZ, pc.scaleX,
                    pc.scaleY, pc.slantX, pc.slantY, pc.m11, pc.m12, pc.m21,
                    pc.m22, pc.opacity, pc.blendMode, ac.posX, ac.posY, ac.posZ,
                    ac.scaleX, ac.scaleY, ac.slantX, ac.slantY, ac.m11, ac.m12,
                    ac.m21, ac.m22, ac.opacity, ac.blendMode, ac.active ? 1 : 0,
                    ac.visible ? 1 : 0);
            }
        }

        // === PHASE 3: Post-loop processing ===
        // Call order matches libkrkr2.so Player_updateLayers
        // (0x6BBC60..0x6BBCA8): sub_6BC000 → sub_6BC4F0 → sub_6BD8DC →
        // sub_6BDA28 → sub_6BDCC0 → sub_6BDE94 → sub_6BE0C0 → sub_6BEDD0 →
