...
namespace motion::detail {
    ...
    namespace {
        ...
        bool isTargetLogoMotionPath(const std::string &motionPath) {
            ...
            return lowered.find("yuzulogo.mtn") != std::string::npos ||
                ...
                lowered.find("title_bg.mtn") != std::string::npos;
        }
        ...
        void collectControlMetadata(MotionSnapshot &snapshot) {
            // metadata.base only identifies the default character/motion.
            // Controller tables (variableList, eyeControl, eyebrowControl,
            // mouthControl, etc.) are siblings of base under metadata.
            const auto metadata =
            ...
            collectVariableListMetadata(metadata, snapshot);
            collectControlBindings(
                metadata, "bustControl", 0,
                { { "var_lr", "var_lr" }, { "var_ud", "var_ud" } }, snapshot);
            collectControlBindings(metadata, "hairControl", 1,
                                   { { "var_lr", "var_lr" },
                                     { "var_lrm", "var_lrm" },
                                     { "var_ud", "var_ud" } },
                                   snapshot);
            collectControlBindings(metadata, "partsControl", 2,
                                   { { "var_lr", "var_lr" },
                                     { "var_lrm", "var_lrm" },
                                     { "var_ud", "var_ud" } },
                                   snapshot);
            collectControlBindings(metadata, "loopControl", 3,
                                   { { "var_loop", "var_loop" } }, snapshot);
            collectControlBindings(metadata, "eyeControl", 4,
                                   { { "label", "label" } }, snapshot);
            collectControlBindings(metadata, "eyebrowControl", 5,
                                   { { "label", "label" } }, snapshot);
            collectControlBindings(
                metadata, "mouthControl", 6,
                { { "label", "label" }, { "talkLabel", "talkLabel" } },
                snapshot);
            collectControlBindings(metadata, "transitionControl", 7,
                                   { { "label", "label" } }, snapshot);
            collectSelectorControlMetadata(metadata, snapshot);
            collectClampControlMetadata(metadata, snapshot);
            collectMirrorControlMetadata(metadata, snapshot);
            collectInstantVariableList(metadata, snapshot);
            collectTimelineControlMetadata(metadata, snapshot);
                        ...
                        for(const auto &priItem : *priorities) {
                            ...
                            if(const auto content =
                                ...
                                for(const auto &item : *content) {
                                    appendResolvedLayerReference(item, snapshot,
                                                                 resolved);
                                }
                            }
                            if(!resolved.empty()) {
    ...
    void mergeAttachedSnapshotResources(MotionSnapshot &primary,
        ...
        if(attached.file) {
            PSB::registerRootResources(
                { widen(attached.path),
                  TVPExtractStorageName(widen(attached.path)) },
                *attached.file);
        }
    }
    ...
    loadMotionSnapshot(const ttstr &path, const tjs_int decryptSeed) {
        ...
        // Some production MTN files are valid motion PSBs without the
        // optional root `id = "motion"` marker used by MotionType inference.
        // The MTN extension is itself an explicit motion-format contract.
        const auto loweredPath = lowercase(path.AsStdString());
        if(file->getType() != PSB::PSBType::Motion &&
           !hasSuffix(loweredPath, ".mtn")) {
            LOGGER->error("this psb file is not a motion/MTN file: {}",
                          path.AsStdString());
            return nullptr;
        }
