...
iTVPTransHandlerProvider *TVPFindTransHandlerProvider(const ttstr &name) {
    ...
    if(!holder) {
        // Extended transitions such as "wave" come from Windows-only
        // plugins (extrans.dll). When the plugin is unavailable, fall
        // back to the built-in crossfade quietly. Saves and scene
        // changes still succeed; only the visual style is simplified.
        static tTJSHashTable<ttstr, tjs_int> loggedMissing;
        if(!loggedMissing.Find(name)) {
            loggedMissing.Add(name, 1);
            TVPAddImportantLog(
                TVPFormatMessage(TVPCannotFindTransHander, name) +
                TJS_W(" (fallback to crossfade)"));
        }
