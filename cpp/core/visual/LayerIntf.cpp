...
tTJSNC_Layer::tTJSNC_Layer() : tTJSNativeClass(TJS_W("Layer")) {
    ...
    TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ stitchWrappedCopy) {
        ...
        auto *destinationBuffer = reinterpret_cast<unsigned char *>(
            _this->GetMainImagePixelBufferForWrite());
        auto *sourceBuffer = reinterpret_cast<const unsigned char *>(
            src->GetMainImagePixelBuffer());
        const tjs_int destinationWidth =
            static_cast<tjs_int>(_this->GetImageWidth());
        const tjs_int destinationHeight =
            static_cast<tjs_int>(_this->GetImageHeight());
        const tjs_int sourceWidth =
            static_cast<tjs_int>(src->GetImageWidth());
        const tjs_int sourceHeight =
            static_cast<tjs_int>(src->GetImageHeight());
        const tjs_int destinationPitch =
            _this->GetMainImagePixelBufferPitch();
        const tjs_int sourcePitch = src->GetMainImagePixelBufferPitch();
        if(!destinationBuffer || !sourceBuffer || destinationWidth <= 0 ||
           destinationHeight <= 0 || sourceWidth <= 0 || sourceHeight <= 0 ||
           destinationPitch == 0 || sourcePitch == 0)
            return TJS_S_OK;

        const tjs_int destinationX =
            param[0]->Type() != tvtVoid ? (tjs_int)*param[0] : 0;
        const tjs_int destinationY =
            param[1]->Type() != tvtVoid ? (tjs_int)*param[1] : 0;
        tjs_int copyWidth =
            param[2]->Type() != tvtVoid ? (tjs_int)*param[2] : -1;
        tjs_int copyHeight =
            param[3]->Type() != tvtVoid ? (tjs_int)*param[3] : -1;
        const tjs_int sourceX =
            param[5]->Type() != tvtVoid ? (tjs_int)*param[5] : 0;
        const tjs_int sourceY =
            param[6]->Type() != tvtVoid ? (tjs_int)*param[6] : 0;
        if(copyWidth <= 0)
            copyWidth = destinationWidth;
        if(copyHeight <= 0)
            copyHeight = destinationHeight;

        const tTVPRect &clip = _this->GetClip();
        const tjs_int copyLeft =
            std::max(std::max(destinationX, clip.left), 0);
        const tjs_int copyTop =
            std::max(std::max(destinationY, clip.top), 0);
        const tjs_int copyRight =
            std::min(std::min(destinationX + copyWidth, clip.right),
                     destinationWidth);
        const tjs_int copyBottom =
            std::min(std::min(destinationY + copyHeight, clip.bottom),
                     destinationHeight);
        if(copyLeft >= copyRight || copyTop >= copyBottom)
            return TJS_S_OK;

        auto positiveModulo = [](tjs_int value, tjs_int divisor) {
            tjs_int remainder = value % divisor;
            return remainder < 0 ? remainder + divisor : remainder;
        };

        for(tjs_int y = copyTop; y < copyBottom; ++y) {
            const tjs_int wrappedY =
                positiveModulo(sourceY + y - destinationY, sourceHeight);
            auto *destinationLine = reinterpret_cast<tjs_uint32 *>(
                destinationBuffer + y * destinationPitch);
            auto *sourceLine = reinterpret_cast<const tjs_uint32 *>(
                sourceBuffer + wrappedY * sourcePitch);
            for(tjs_int x = copyLeft; x < copyRight; ++x) {
                destinationLine[x] =
                    sourceLine[positiveModulo(
                        sourceX + x - destinationX, sourceWidth)];
            }
        }

        _this->Update(tTVPRect(copyLeft, copyTop, copyRight, copyBottom));
        if(result)
            *result = 1;
        return TJS_S_OK;
    }
