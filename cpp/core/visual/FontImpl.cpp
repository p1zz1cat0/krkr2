...
static void TVPAddFontAlias(const ttstr &alias, const ttstr &family) {
    if(TVPFontNames.Find(alias))
        return;
    if(auto *info = TVPFontNames.Find(family))
        TVPFontNames.Add(alias, *info);
}

static void TVPAddCJKFontAliases() {
    // A number of translated KAG games refer to Source Han by its localized
    // family name while omitting the original font files.  KrKr2 already
    // bundles the metrically compatible Noto Sans CJK collection, so expose
    // conservative aliases instead of falling back to an arbitrary TTC face.
    const ttstr sc = TJS_W("Noto Sans CJK SC");
    TVPAddFontAlias(TJS_W("思源黑体"), sc);
    TVPAddFontAlias(TJS_W("思源黑体R"), sc);
    TVPAddFontAlias(TJS_W("思源黑体B"), sc);
    TVPAddFontAlias(TJS_W("思源黑体H"), sc);
    TVPAddFontAlias(TJS_W("Source Han Sans SC"), sc);

    const ttstr tc = TJS_W("Noto Sans CJK TC");
    TVPAddFontAlias(TJS_W("思源黑體"), tc);
    TVPAddFontAlias(TJS_W("思源黑體R"), tc);
    TVPAddFontAlias(TJS_W("思源黑體B"), tc);
    TVPAddFontAlias(TJS_W("思源黑體H"), tc);
    TVPAddFontAlias(TJS_W("Source Han Sans TC"), tc);
}
...
tjs_uint TVPGetFontFaceIndex(const ttstr &fontname) {
    TVPFontNamePathInfo *info = TVPFindFont(fontname);
    if(!info)
        info = TVPFontNames.Find(TVPDefaultFontName);
    if(!info || info->Index < 0)
        return 0;
    return static_cast<tjs_uint>(info->Index);
}
...
void TVPInitFontNames() {
    ...
    TVPAddCJKFontAliases();

    // Prefer the proportional Japanese face as the stable engine fallback.
    // Explicit SC/TC family names and aliases retain their own TTC index.
    if(TVPFontNames.Find(TJS_W("Noto Sans CJK JP")))
        TVPDefaultFontName = TJS_W("Noto Sans CJK JP");
