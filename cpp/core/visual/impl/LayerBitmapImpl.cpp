...
static bool TVPPrerenderedFontCoversText(tTVPPrerenderedFont *font,
                                         const ttstr &text) {
    if(!font)
        return false;

    const tjs_char *character = text.c_str();
    while(*character) {
        if(!font->Find(*character))
            return false;
        ++character;
    }
    return true;
...
void tTVPNativeBaseBitmap::DrawTextMultiple(
    ...
    // Never mix a pre-rendered font with the native fallback within one text
    // run. The fonts can have different glyph shapes, weights and metrics;
    // translated strings containing characters absent from a Japanese TFT
    // otherwise look like corrupted glyphs. If the TFT cannot cover the
    // complete run, render the complete run with the native rasterizer.
    tTVPPrerenderedFont *runPrerenderedFont =
        TVPPrerenderedFontCoversText(PrerenderedFont, text)
            ? PrerenderedFont
            : nullptr;
    ...
    while(*p) // while input string is remaining
    {
        ...
        tTVPCharacterData *shadow = nullptr;
        try {
            data = TVPGetCharacter(font, this, runPrerenderedFont, AscentOfsX,
                                   AscentOfsY);
            ...
            if(data) {
                if(shlevel != 0) {
                    if(shlevel == 255 && shwidth == 0) {
                        ...
                        // blured shadow
                        font.Blured = true;
                        shadow = TVPGetCharacter(font, this, runPrerenderedFont,
                                                 AscentOfsX, AscentOfsY);
                    }
                }
...
void tTVPNativeBaseBitmap::GetTextSize(const ttstr &text) {
    ...
    if(text != CachedText) {
        ...
        if(TVPPrerenderedFontCoversText(PrerenderedFont, text)) {
            tjs_uint width = 0;
