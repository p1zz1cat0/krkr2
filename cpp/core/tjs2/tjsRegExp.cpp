...
namespace TJS {
    ...
    static bool TJSIsHexDigit(tjs_char ch) {
        return (ch >= TJS_W('0') && ch <= TJS_W('9')) ||
            (ch >= TJS_W('a') && ch <= TJS_W('f')) ||
            (ch >= TJS_W('A') && ch <= TJS_W('F'));
    }

    // TJS historically accepts unbraced four-digit Unicode escapes such as
    // \\x3042 in regular expressions. Modern Oniguruma interprets \\x as a
    // two-digit encoded-byte escape, which is invalid for values like FF in a
    // UTF-16 pattern. Convert only complete legacy four-digit escapes to the
    // equivalent code-point form understood by current Oniguruma. Escaped
    // backslashes remain literal.
    static ttstr TJSNormalizeLegacyUnicodeEscapes(const tjs_char *start,
                                                   const tjs_char *end) {
        std::basic_string<tjs_char> normalized;
        normalized.reserve(static_cast<size_t>(end - start));

        for(const tjs_char *p = start; p < end;) {
            if(*p == TJS_W('\\') && p + 1 < end && p[1] == TJS_W('\\')) {
                normalized.append(p, p + 2);
                p += 2;
                continue;
            }

            if(*p == TJS_W('\\') && p + 5 < end && p[1] == TJS_W('x') &&
               TJSIsHexDigit(p[2]) && TJSIsHexDigit(p[3]) &&
               TJSIsHexDigit(p[4]) && TJSIsHexDigit(p[5]) &&
               (p + 6 == end || !TJSIsHexDigit(p[6]))) {
                normalized += TJS_W('\\');
                normalized += TJS_W('x');
                normalized += TJS_W('{');
                normalized.append(p + 2, p + 6);
                normalized += TJS_W('}');
                p += 6;
                continue;
            }
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ _compile) {
    ...
    */
    ...
    try {
        ...
        const ttstr pattern = TJSNormalizeLegacyUnicodeEscapes(
            exprstart, expr.c_str() + expr.length());
        OnigErrorInfo einfo;
        int r = onig_new(&(_this->RegEx), (UChar *)pattern.c_str(),
...
void tTJSNC_RegExp::Compile(tjs_int numparams, tTJSVariant **param,
    ...
    const ttstr pattern = TJSNormalizeLegacyUnicodeEscapes(
        expr.c_str(), expr.c_str() + expr.length());
    OnigErrorInfo einfo;
    int r = onig_new(&(_this->RegEx), (UChar *)pattern.c_str(),
                     (UChar *)(pattern.c_str() + pattern.length()),
                     flags & ((ONIG_OPTION_MAXBIT << 1) - 1),
