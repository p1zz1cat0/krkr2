...
//---------------------------------------------------------------------------
tTVPWaveSoundBufferThread::~tTVPWaveSoundBufferThread() {
    // Signal the worker before joining it.  The upstream order waited for a
    // still-running thread and only called Terminate() after WaitFor(), which
    // deadlocked every normal engine shutdown once audio had been initialized.
    Terminate();
