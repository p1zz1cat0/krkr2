...
void TVPMainScene::doStartup(float dt, std::string path) {
    ...
    bool showFps = pGlobalCfgMgr->GetValue<bool>("showfps", false);
#if CC_TARGET_PLATFORM == CC_PLATFORM_MAC
    // Android-oriented port packages sometimes ship with showfps enabled in
    // their global configuration.  That debug overlay then obscures the
    // native macOS game window even though the user never requested it.
    // Keep an explicit escape hatch for runtime diagnostics.
    const char *showFpsOverride = std::getenv("YOGHOURT_KRKR_SHOW_FPS");
    showFps =
        showFpsOverride != nullptr &&
        (showFpsOverride[0] == '1' || showFpsOverride[0] == 'y' ||
         showFpsOverride[0] == 'Y' || showFpsOverride[0] == 't' ||
         showFpsOverride[0] == 'T');
#endif
    if(showFps) {
        _fpsLabel =
