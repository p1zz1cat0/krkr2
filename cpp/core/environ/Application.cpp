...
void tTVPApplication::Run() {
    try {
        ...
        //	TVPBreathe();
        ProcessMessages();
        // System.terminate() is asynchronous and can set TVPTerminated while
        // ProcessMessages() is dispatching a script event.  Some games remove
        // or hide their final window in that same event, so Cocos may stop
        // scheduling this method and the original entry-only check is never
        // reached again.  Finish shutdown after the event returns, while the
        // engine is still in a valid frame and can clean up its worker threads.
        if(TVPTerminated) {
            TVPSystemUninit();
            TVPExitApplication(TVPTerminateCode);
        }
