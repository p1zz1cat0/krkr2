...
bool TVPCheckStartupArg() {
    ...
    std::string dir;
    bool isArchive = fs::is_regular_file(entry) && fs::path(entry).extension() == ".xp3";

    if(fs::is_directory(entry)) {
        dir = entry;
    } else if(isArchive) {
        dir = fs::path(entry).parent_path().string();
    } else {
        return false;
    }
    ...
    for(int i = 2; i < argc; ++i) {
        ...
        if(pos == std::string::npos) {
            TVPSetCommandLine(ttstr(arg).c_str(), ttstr("yes"));
        } else {
            TVPSetCommandLine(ttstr(arg.substr(0, pos)).c_str(),
                              ttstr(arg.substr(pos + 1)));
        }
    }

    // Track whether startup.tjs lives inside the XP3 (only set when
    // we fall back to TVPCheckArchive).
    bool startupInsideArchive = false;
    ...
    TVPListDir(dir, [&](const std::string &name, int mask) {
        if(mask & S_IFREG) {
            ...
            if(fs::path(lower).filename() == "startup.tjs") {
                bootable = true;
            }
        }
    });

    // When the entry is an XP3 archive and startup.tjs is not a
    // loose file next to it, check inside the archive itself
    // (matching the Windows TVPCheckArchive path).
    if(!bootable && isArchive) {
        int result = TVPCheckArchive(ttstr(entry));
        if(result == 1) {
            bootable = true;
            startupInsideArchive = true;
        }
    }

    if(bootable) {
        if(startupInsideArchive) {
            // startup.tjs lives inside the XP3; use the archive as
            // the startup entry so the engine can find it.  The
            // game's own startup.tjs is responsible for registering
            // sibling XP3s via Storages.addAutoPath().
            TVPMainScene::GetInstance()->startupFrom(entry);
        } else {
            // startup.tjs is a loose file in the game directory.
            // Use the directory (not the XP3) so that TVPProjectDir
            // covers the whole game folder — otherwise the engine
            // locks itself inside a single archive and cannot
            // resolve assets that live outside it (e.g.
            // yuzulogo.psb in a different XP3 or loose on disk).
