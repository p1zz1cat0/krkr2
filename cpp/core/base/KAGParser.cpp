...
class tKAGTagMemberCollector final : public tTJSDispatch {
public:
    ...
    tjs_error FuncCall(tjs_uint32, const tjs_char *, tjs_uint32 *,
        ...
        if(numparams >= 2 && !((tjs_int)*param[1] & TJS_HIDDENMEMBER)) {
            ttstr name(*param[0]);
            if(name != TJS_W("tagname") && name != TJS_W("taglist"))
...
void TVPAttachKAGTagList(iTJSDispatch2 *tag) {
    auto *collector = new tKAGTagMemberCollector();
    tTJSVariantClosure callback(collector, nullptr);
    tag->EnumMembers(TJS_IGNOREPROP | TJS_ENUM_NO_VALUE, &callback, tag);
    std::sort(collector->Names.begin(), collector->Names.end());

    iTJSDispatch2 *list = TJSCreateArrayObject();
    tTJSVariant tagName(TJS_W("tagname"));
    list->PropSetByNum(TJS_MEMBERENSURE, 0, &tagName, list);
    for(size_t i = 0; i < collector->Names.size(); ++i) {
        tTJSVariant name(collector->Names[i]);
        list->PropSetByNum(TJS_MEMBERENSURE, static_cast<tjs_int>(i + 1),
                           &name, list);
    }
    tTJSVariant value(list, list);
    tag->PropSet(TJS_MEMBERENSURE, TJS_W("taglist"), nullptr, &value, tag);
    list->Release();
    collector->Release();
}

iTJSDispatch2 *TVPReturnKAGTag(iTJSDispatch2 *tag) {
    TVPAttachKAGTagList(tag);
    tag->AddRef();
    return tag;
}
                    ...
                    if(tagkind == tag_emb ||
                       (ismacro && tagkind == tag_other)) {
                        ...
                        if(!ismacro) {
                            ...
                            bool escape = true;
                            static ttstr escape_name(
                                TJSMapGlobalStringMap(TJS_W("escape")));
                            if(TJS_SUCCEEDED(DicObj->PropGet(
                                   0, escape_name.c_str(),
                                   escape_name.GetHint(), &val, DicObj))) {
